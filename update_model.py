...
def main():
    parser = argparse.ArgumentParser(description="Manage models in a directory.")
    parser.add_argument("action", choices=["add", "remove"], help="Action to perform: 'add' or 'remove' a model.")
    parser.add_argument("-p","--filepath", metavar="FILEPATH",  type=str, help="Filepath of the model.")
    ...
    if not os.path.exists(os.path.join(modelzoo_location,'models.json')):
        raise FileNotFoundError(f'No models.json found in {os.path.join(modelzoo_location,"models.json")} which is required.')
    ...
    if args.action == "add":
        ...
        destination_path = os.path.join(modelzoo_location, 'models', os.path.basename(filepath))
        try:
        ...
        try:
            os.remove(filepath)
            print(f"File removed successfully from {os.path.dirname(filepath)}.")
        except Exception as e:
